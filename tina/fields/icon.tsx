...
export const IconPickerInput = wrapFieldsWithMeta(({ input }) => {
    ...
    const InputIcon = (IconOptions as any)[input.value]
        ? (IconOptions as any)[input.value]
        : null;
                ...
                {({ open }) => (
                    <>
                        <PopoverButton>
                            ...
                            >
                                {InputIcon && (
                                    <InputIcon className="w-7 mr-1 h-auto fill-current text-amber-500" />
                                )}
                                {inputLabel}
                                {!InputIcon && (
                                        ...
                                        <div className="max-h-[24rem] flex flex-col w-full h-full">
                                            ...
                                            {filteredBlocks.length > 0 && (
                                                <div className="w-full grid grid-cols-6 auto-rows-auto p-2 overflow-y-auto">
                                                    <button
                                                        className="relative rounded-lg text-center text-xs py-2 px-3 flex-1 outline-none transition-all ease-out duration-150 hover:text-amber-500 focus:text-amber-500 focus:bg-gray-50 hover:bg-gray-50"
                                                        key={"clear-input"}
                                                        onClick={() => {
                                                            input.onChange("");
                                                            setFilter("");
                                                    ...
                                                    {filteredBlocks.map(
                                                        (name) => {
                                                            return (
                                                                <button
                                                                    className="relative flex items-center justify-center rounded-lg text-center text-xs py-2 px-3 flex-1 outline-none transition-all ease-out duration-150 hover:text-amber-500 focus:text-amber-500 focus:bg-gray-50 hover:bg-gray-50"
                                                                    key={name}
                                                                    onClick={() => {
                                                                        input.onChange(
                                                                            name
